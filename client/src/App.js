...
function App() {
    ...
    void mainImage(out vec4 FragColor){
        vec4 buff1 = texture(iBufferTexture1, 2. *gl_FragCoord.xy/iResolution);
        vec4 buff2 = texture(iBufferTexture2, 2. *gl_FragCoord.xy/iResolution);
        vec4 buff3 = texture(iBufferTexture3, 2. *gl_FragCoord.xy/iResolution);
        vec4 buff4 = texture(iBufferTexture4, 2. *gl_FragCoord.xy/iResolution);
        float xStep = step(0.5,gl_FragCoord.x/iResolution.x);
        vec4 top = xStep * buff1 + (1.0 - xStep) * buff2;
        vec4 bottom = xStep * buff3 + (1.0 - xStep) * buff4;
        float yStep = step(0.5,gl_FragCoord.y/iResolution.y);
        FragColor = yStep * top + (1.0 - yStep) * bottom;
        // vec4 channel0 = texture(iChannel0, gl_FragCoord.xy/iResolution);
        // FragColor = vec4(channel0.xyz, 1.0);
    ...
    return (
        <div>
            <GraphicsComponent
                ...
                channels={[[{ type: "sampler2D", path: "textures/city_street.jpg" }, null, null, null], [null, null, null, null], [null, null, null, null], [null, null, null, null], [null, null, null, null]]}
